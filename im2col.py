...
__global__ void im2col_gpu_kernel_batched(const int n, const float* data_im, const int height, const int width, const int ksize, const int pad, const int stride, const int height_col, const int width_col, const int offset, float* data_col) {
    int y_offset = blockIdx.y * blockDim.x * gridDim.x;
    data_col += y_offset;
    //printf("y_offset %d, height %d, width %d, height_col %d, width_col %d\\n", y_offset, height, width, height_col, width_col);
    data_im += offset + y_offset;
    CUDA_KERNEL_LOOP(index, n) {
        int w_out = index % width_col;
        index /= width_col;
        int h_out = index % height_col;
...
def compute_im2col_batched(in_array, window_height, window_width, window_channels, ksize, pad, stride, start_idx, batchsize):
    ...
    gridsize = ((num_kernels+threads -1)/threads, batchsize, 1)
    result = gpu.empty((batchsize, ksize*ksize*window_channels, height_col*width_col), np.float32)

    im2col_batched(num_kernels, in_array, np.int32(height), np.int32(width), np.int32(ksize), np.int32(pad), np.int32(stride), height_col, width_col, np.int32(start_idx), result, block=blocksize, grid=gridsize)
    return result
...
def test_batched():
    ...
    A_d = gpu.to_gpu(A)
    result = compute_im2col_batched(A_d, 4, 4, 2, ksize, pad, stride, offset, batchsize)
    print A_d
    print result
